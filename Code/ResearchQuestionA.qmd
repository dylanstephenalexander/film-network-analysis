...
#Some movies have multiple genres, I decided to just keep the first genre mentioned for each movie to maintain consistency and avoid hundreds of possible genre combinations.
```
...
colnames(tbl1) <- c("Title", "Number of People")
...
colnames(profession_summary) <- c("Main Profession", "Number of People", "Mean Degree", "Mean Eigenvector")

#profession_summary
h <- na.omit(head(profession_summary,6)) #Table 2
t <- tail(profession_summary,5) #Table 3

prof_cent_df <- rbind(h,t)

#library(webshot2)

#save_kable(
#  kbl(prof_cent_df, format = "html") %>% 
#    kable_classic(),
#  file = "prof_cent_df.png"
#)

prof_cent_df$`Mean Eigenvector` <- format(
  prof_cent_df$`Mean Eigenvector`,
  scientific = TRUE,
  digits = 3
)

prof_cent_df$`Mean Degree` <- format(
  prof_cent_df$`Mean Degree`,
  digits = 3
)
...
kbl(prof_cent_df, 
    format = "latex", 
    booktabs = TRUE,
    #caption = "Head and Tail of Profession Structure Summary"
) %>%
  kable_styling(latex_options = c("hold_position"))
...
h <- head(profession_struct_summary, 5) #Table 4
t<- tail(profession_struct_summary, 5) #Table 5
prof_cluster_df <- rbind(h, t)
...
#genre_summary
h <- head(genre_summary, 5) #Table 6
t <- tail(genre_summary, 5) #Table 7

genre_cent_df <- rbind(h, t)

genre_cent_df$`Mean Eigenvector` <- format(
  genre_cent_df$`Mean Eigenvector`,
  scientific = TRUE,
  digits = 3
)

genre_cent_df$`Mean Degree` <- format(
  genre_cent_df$`Mean Degree`,
  digits = 3
)
...
kbl(genre_cent_df, 
    format = "latex", 
    booktabs = TRUE,
    #caption = "Head and Tail of Genre Structure Summary"
) %>%
  kable_styling(latex_options = c("hold_position"))
```
...
h <- head(profession_struct_summary3, 5) #Table 8
t <- tail(profession_struct_summary3, 5) #Table 9

genre_cluster_df <- rbind(h, t)

kbl(genre_cluster_df, 
    format = "latex", 
    booktabs = TRUE,
    #caption = "Head and Tail of Genre Structure Summary",
    digits = 3
